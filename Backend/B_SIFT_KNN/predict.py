...
def extract_regions(image_path):
    ...
    image = read_image(image_path, display=True)
...
def equalize_iris_region(image, outer_circle, inner_circle):
    def locate_roi():
        mask = image.copy()
        mask[:] = (0)
...
def segment_regions(image, inner_circle, outer_circle, display=False):
    ...
    if display:
        ...
        ax.imshow(regions['entire']['image'], cmap='gray')
        ax.set_title('Entire')
        ax.set_xticks([])
        ax.set_yticks([])
        ...
        plot_image_np = np.array(Image.open(buf))  # Convert the PIL image to numpy array while the buffer is still open
        
        buf.close()
        plot.close(fig)

    return regions, plot_image_np



def detect_keypoints(sift_engine, regions):    
    ...
    regions['entire']['image_with_keypoints'] = image_with_keypoints
...
def match_all_regions2(regions_one, regions_two, ratio_threshold, angle_standard_deviation, distance_standard_deviation, display=False):
    if display:
        plot.figure(figsize=(10, 5))
        plot.title('Keypoint Matches')
...
def perform_visual_comparison( predicted_class , serialized_data_path, test_regions):
    raw_data_path = "Backend\A_Data\Raw_Data"
    ...
    save =match_all_regions2(test_regions, serialized_data, 0.8, 10, 0.15, display=True)
    if save is not None:
        save_path = "Temp/image_matches.png"
        plot.imsave(save_path, cv2.cvtColor(save, cv2.COLOR_BGR2RGB))
        print(f"Saved comparison image to {save_path}")

def find_best_match(test_data):

    serialized_data_path = "Backend\B_SIFT_KNN\Model"
    ...
    if best_match:
            ...
            perform_visual_comparison(best_match , serialized_data_path, test_data )
            return best_match  , prediction_time
    else:
